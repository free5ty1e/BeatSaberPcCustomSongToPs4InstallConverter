...
**Status:** 🟡 **EXP 200 (v0.5328 REDEPLOY IN PROGRESS): Camellia/Chromeo gameplay crash root-caused + fixed.** User verified full-fleet deploy: boot clean, RS/lizzo/billieeilish customs all played NoArrows fine; Chromeo selection crashed CE-34878-0. Root cause: reconstructed V4-to-V3 Chromeo beatmaps were minimal-schema (8 keys vs the required 17-key V3 set) + 3 slots had zero-note Easy maps; game dies deserializing at gameplay load (log ends right after level-file open, before environment/audio opens). v0.5328 adds normalize_v3_schema() at both injection points + empty-difficulty rescue via populated Standard donor; 571/571 tests incl. Roni-source regression. Full rebuild+redeploy running via build_deploy_all38.py. Next: user re-tests Chromeo, then fresh-pack generalization test.
