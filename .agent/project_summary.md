...
**Exp 135 (2026-07-15) — ❌ CRASHED:** LZ4HC-rebuilt bundle (flag=3) also crashed. Compression flags NOT the issue.
**Exp 136 (2026-07-15) — ✅ ROOT CAUSE IDENTIFIED:** Catalog CRC check (m_UseCrcForCachedBundles=true) causes crash on any modified bundle.

**New approach:** Bypass pack bundle entirely. Test if `--enable-modes` works on PS4 via per-song bundle (doesn't require pack bundle modification). **Exp 138 (2026-07-15) — ⏳ BLOCKED (PS4 offline):** Deploy startmeup_custom_v3_modes.bundle to test mode selector. **What works:** Per-song bundle redirect. Plugin toggle. Pipeline song building with PCM16 audio.
...
## 2026-07-17: CRC Validation Breakthrough Achieved

**Major milestone:** Solved the Addressables catalog CRC validation blocker that has blocked all pack bundle modification attempts since Experiment 136.

### What Works Now
- ✅ **CRC matching via GF(2) linear algebra**: 9 alignment padding bytes at offset 263 can be computed to produce exact CRC match (`0xdc8b314f`)
- ✅ **Espresso BeatmapLevelSO blob builder**: Creates 1,257-byte blob with 5 preview modes (Standard, OneSaber, NoArrows, 90Degree, 360Degree)
- ✅ **Patched pack bundle**: `rollingstones_pack_patched.bundle` (7,905,515 bytes, CRC=`0xdc8b314f`)

### Current State
- Bundle deployed to PS4 via AFR redirect
- AWAITING USER TEST for display name + mode selector functionality
- If size validation blocks: need uncompressed block injection approach (6.1 MB free variables)
- If pathIDs invalid: need to fix BeatmapCharacteristicSO references

### Next Steps
1. Test on PS4 (user will power on soon)
2. Based on results: either deploy Espresso replacement OR investigate alternative approaches
3. Document solution in knowledge base if successful

### Key Files
- `/workspace/beat_saber_deluxe/tools/build_patched_pack_bundle.py` — Working CRC correction pipeline
- `/workspace/beat_saber_deluxe/rollingstones_pack_patched.bundle` — Tested bundle (CRC matched)
- `/workspace/.agent/plans/crc-blocker-and-display-fix.md` — Comprehensive approach plan

### Critical Insight from Exp 142 Test Results
"CRC check PASSED (log shows game continued loading other bundles after pack bundle). Crash likely from either (a) m_BundleSize validation or (b) invalid BeatmapCharacteristicSO pathIDs."
