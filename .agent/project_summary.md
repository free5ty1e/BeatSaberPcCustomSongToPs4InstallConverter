...
**Status:** 🟢 **EXP 233 (pipeline v0.5350): the REAL 3-file deletion bug fixed (relative-vs-absolute Path comparison) + --no-prompt + last CI gates. QA GREEN (user full clean-slate procedure passed).** The tracked custom_songs/ files kept dying because the wipe compared git's REPO-RELATIVE paths against rglob's ABSOLUTE paths — never equal, so every file was unlinked while the log said "preserved". Both prior fixes tested the helper, not its consumer. Now: helper returns resolved-absolute paths; wipe compares as string sets; NEW end-to-end regression test runs the real wipe against a scratch git repo. All 34 example scripts take --no-prompt (unattended chained runs). Last CI red was one live-state test class — gated like the rest. PR description surgically updated (pending user approval to push).

**Prior (EXP 232 (pipeline v0.5349): CRITICAL chained-deploy regression fixed + live PS4 state repaired.** Clean-slate + 5-script chain left only the LAST pack (RS) working — the other 4 showed swapped metadata but stock bundles. Root cause: _ensure_pack_bundle_redirects' stale-sweep validated "still configured" by LOCAL bundle existence while the clean-slate wipe empties pack_modes_bundles/ — each script's deploy deleted the previous packs' redirects as "no longer configured" (bundles were live on the PS4). LATENT SINCE Exp 226 — prior chained QA passed only because stale local bundles made the filter accidentally permissive; the Exp 227 true-clean-slate exposed it (proven: HEAD v0.5347 reproduces identically). Fix: stale-sweep validity = current pair OR deployed-per-state-file (_resolve_deployed_packs). Live state repaired: 5 pack pairs + catalog re-added (validation PASSED, 36 songs), 2 ghost PATD song redirects cleared. 3 regression tests pin the exact chain.

**Prior (EXP 231 (pipeline v0.5348): v4-columnar NoArrows clobber fixed + the recurring 3-file deletion truly fixed.** '15 Minutes' Hard NoArrows showed arrows — NOT a missing-difficulty failure (443f3 provides all 5 natively incl. ExpertPlus via tier-3; generation ran correctly): the v4 columnar rows' arrow d-values clobbered the generator's dots via convert_v4_to_v3's row-over-event merge at injection. Fix: re-apply the NoArrows dot pass AFTER format conversion (idempotent; 90Degree deliberately excluded — rotation-append not idempotent). OneSaber was already immune; 90Degree verified immune. Both v4 songs (15 Minutes + Oxytocin) redeployed — all 10 NoArrows charts now all-dots, verified in-bundle + live on PS4. The 3 tracked custom_songs/ files kept dying because the Exp 229 preserve-fix was cwd-dependent (git ls-files inherited process cwd; any non-repo launch dir → [] → wholesale rmtree) — now pinned with `git -C <script_dir>`; verified from /workspace, /tmp, /. 639/639 tests.

**Prior (EXP 230 (pipeline v0.5347): lint fixes in code — user-approved post-QA pass pending.** All 17 ruff findings in full_custom_song_pipeline.py resolved IN CODE (2 dead stores, 3 AST-verified-unused imports, 10 f-string prefixes, 2 import sorts — zero functional changes, normalize_v3_schema proven byte-identical across 10 edge cases); the Exp 229 per-file-ignores suppression removed — file fully lint-enforced, `ruff check tools/` exit 0. Version 0.5346 → 0.5347 (plugin unchanged v0.8047). User will run the full clean-slate test procedure once more for a stable release point.

**Prior (EXP 229 (docs/tests/CI only; pipeline stays v0.5346 — QA-FROZEN): CI failure RCA + release artifact audit — release-ready.** GitHub CI had 214 test failures (absolute /workspace paths that don't exist on runners + hardware-environment tests with no skip guards) and a lying-green Unit Tests job (tee swallowed pytest exit). All fixed: repo-relative test paths, HAS_DUMP/HAS_BUILT_ALL skip gates, set -o pipefail, 17 ruff findings resolved via pyproject.toml per-file-ignores (pipeline code untouched — QA-frozen per user directive). Release zip now ships ALL 69 example_*.* files + song catalog + requirements + config template; CI_RELEASE.md + README state the user-supplied ps4_dump/ requirement (no game data in the release). PR #4 title/description updated to the true branch scope (Exps 160-229). --clean-ps4 wipe now preserves git-tracked assets. User's clean-slate 5-pack retest passed end-to-end; staged for their push to re-trigger CI.

**Prior (EXP 228 (pipeline v0.5346 / plugin v0.8047): Unicode metadata matching + BeatSaver V4 map support.** '…Baby One More Time' never got its title replacement — 3-part chain: pipeline wrote the key `…`-escaped, plugin parsed bytes verbatim, and the game's UTF-16 title folds to `?Baby One More Time` in extract_utf16_string. Fix: song_metadata.json now raw UTF-8 (ensure_ascii=False); plugin unescapes `\uXXXX` AND folds keys through the same '?' projection (fold_utf8_to_ascii — BMP 1×`?`, astral 2×`?`); create_il2cpp_string decodes UTF-8→UTF-16LE properly. Same RCA surfaced: two deployed songs (Oxytocin/Crown) were BeatSaver v4.0.x maps — the PS4's native columnar format parses fine (Standard charts played), but the mode generators produce AMBIGUOUS mixed events on columnar input, and V4 Info.dat's song.title/author/audio.bpm keys were unread (display names fell back to map IDs '4dea2'/'443f3'). New is_v4_beatmap/convert_v4_to_v3 denormalizer + --metadata-only CLI mode; both v4 bundles regenerated+redeployed, metadata corrected live. 634/634 tests.

**Prior (Exp 227, pipeline v0.5345): ghost packs eliminated + clean slate now truly clean.** The Exp 226 union added the LOCAL-BUNDLE fallback into deploy scopes — a clean-slate clear-song run rebuilt/deployed 6 never-installed packs (stale bundles in pack_modes_bundles/ from earlier repairs). Deploy flows now use _resolve_deployed_packs (redirects.json state ONLY); caller-provided packs= is authoritative in entries/ensure. --clean-ps4 now wipes ALL local state per user directive (3 configs + pack_modes_bundles/ + custom_songs/ + mass_bundles/ + blobs — all regenerable). Cross-pack preservation verified intact. 616/616 tests.

**Prior (Exp 226, pipeline v0.5344): mode buttons for EVERY modified pack + two live-state disasters fixed.** (1) Pure-filter pack scoping meant each per-song deploy skipped the requested pack's patch if deployed-state discovery hadn't seen it yet — only the first-deployed pack got mode buttons. Requested packs now JOIN the active set in all 4 helpers; all 5 packs patched + validated live. (2) --clear-target-song on the last custom of an unrelated pack wiped ALL 44 song redirects (empty-scope wipe branch) — fixed 3 ways, live state restored from deployed bundles, validation PASSED. (3) RS script carried a misplaced PATD slot (SugarSoaker) — removed; PATD deployment reverted. Scripts completed: BE +3 songs (Mr. Brightside/Kiss Me More/15 Minutes), RS +Wake Me Up; britney Toxic/Womanizer slots moved off the target artist (Complicated/Teenagers). 613/613 tests.

**Prior (Exp 225, pipeline v0.5343): clean-slate CE-34878-0 crash fixed — Exp 224 regression.** The v0.5342 removal of the hardcoded pack list left two gates checking the (now always-empty) pinned list: on a clean-slate first-song deploy the patched PACK bundle shipped WITHOUT its merged catalog or the aa/catalog.json redirect → game read origin catalog → Unity CRC crash (classic Exp 180 signature, confirmed in bs_log: catalog opened UNREDIRECTED at OPEN #62, patched pack REDIRECTED at OPEN #203). Gates now use _resolve_active_packs; failed validation now exits 1 (no more green-looking broken deploys); 4 regression tests pin the exact crash path. Live PS4 repaired (catalog deployed, 3 redirects, validation PASSED). 607/607 tests.

**Prior (Exp 224, pipeline v0.5342): the `&` path bug + hardcoded pack expectations REMOVED (user directive).** lftp splits unquoted paths at `&` — the Scream&Shout upload silently failed while lftp exited 0; all 21 FTP command sites now quote paths (_ftp_quote) and deploy_to_ps4 verifies the remote listing. The Exp 188-era hardcoded pack_modes.packs default (therollingstones/billieeilish/lizzo/camellia) is GONE — the active pack set is auto-discovered from what the user actually deployed (redirects.json state), with user-pinned lists still supported. Scream&Shout_v3.bundle re-uploaded + full validation GREEN (catalog md5 match, 3 deployed packs, zero missing). 603/603 tests.

**Prior (Exp 222 (plugin v0.8046, pipeline v0.5341): feature-flag gating audit — enable_beatmap_mode_mapping was DECORATIVE, now genuinely gates.** User asked why the mode selector worked with the flag absent — because the flag gated nothing (read, logged, counted; zero conditional uses). v0.8046: when OFF, open-hook skips pack_assets + catalog redirects (stock packs → Standard-only modes, per-song customs still play; catalog skip required by the Exp 180 CRC invariant). All four flags audited: kill switch, song replacements, metadata — all real; gating contract now pinned by TestFeatureFlagGatingAudit (test_no_flag_is_decorative fails CI for future decorative flags). Also fixed ps4_state.py "Extra data" JSON parse (lftp banner chatter around cat output → raw_decode one object). 603/603 tests.

**Prior (Exp 221, plugin v0.8045, pipeline v0.5340): flag-count mystery + deploy guarantees.** The "2/3 feature flags" report was a stale local features.json shipped without enable_beatmap_mode_mapping — deploys now merge DEFAULT_FEATURES (missing keys materialized, explicit values preserved). --deploy-full always builds+deploys the LATEST plugin (opt out: --skip-plugin-deployment). Boot notification compressed to ONE toast — "BS Deluxe v0.8045 (ON/OFF) / (N/3 features ON)" — because PS4VR headset switch-over swallows a second toast. ps4_state.py now reports PS4 feature flags. Deployed + verified both states; PS4 restored to all-ON. 598/598 tests. Second toast after the version banner: "BSD Plugin enabled — N/3 feature flags ON" or "BSD Plugin DISABLED — Official songs only", so the plugin's overall state is visible from the PS4 home screen. Deployed + strings-verified (downloaded .prx bytes carry v0.8044; note GoldHEN FTPD unpacks FSELF on download — md5 compare is invalid for .prx verification). 595/595 tests.

**Prior (Exp 219, pipeline v0.5339, plugin v0.8043): example-file song-quality audit + global kill switch.** Audited all 44 unique BeatSaver MAP_IDs across the 34 pack example docs against the live API: 15 failed the native Easy/Normal/Hard rule (per user_preferences.md — Expert-heavy maps lock out lower-skilled players; fill-up of Expert/E+ is fine, Easy/Normal/Hard must be native). All replaced with verified-qualifying maps — each candidate was DOWNLOADED and its per-difficulty note counts checked for real progressions (several API-passing candidates rejected for duplicate/flat charts, e.g. Gangnam 2ed00 Easy=Normal=Hard=544). Camelia pack now: Le Freak (1760d), 24K Magic (16726), Fireball (9c05), Around The World (40201), Daft Punk Megamix 1 (242e9), Stayin' Alive (3cfe9) — all 5/5 native. Difficulty comments across all docs now state the TRUE native diff set (old docs claimed "5/5" for ExpertPlus-only maps). NEW global kill switch: `--features-only --set-feature enable_plugin=false` makes the plugin fully inert on next boot (official songs only, no plugins.ini edit); `enable_plugin` defaults TRUE when absent. 595/595 tests.

**Prior (Exp 218, v0.5338):** song-quality fixes, hardware-verified — fill_missing_standard_difficulties (no stock beatmaps in missing-diff slots), Info.dat BPM as the authoritative bpmData grid, OneSaber = blue dots. User confirmed clean-slate install works end to end.
...
**Plugin v0.8043:** `enable_plugin` global kill switch + `enable_beatmap_mode_mapping` runtime flag.
