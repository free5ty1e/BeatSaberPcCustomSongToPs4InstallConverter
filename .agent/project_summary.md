...
**Status:** 🟢 **EXP 220 (plugin v0.8044): startup flag-status notification.** Second toast after the version banner: "BSD Plugin enabled — N/3 feature flags ON" or "BSD Plugin DISABLED — Official songs only", so the plugin's overall state is visible from the PS4 home screen. Deployed + strings-verified (downloaded .prx bytes carry v0.8044; note GoldHEN FTPD unpacks FSELF on download — md5 compare is invalid for .prx verification). 595/595 tests.
