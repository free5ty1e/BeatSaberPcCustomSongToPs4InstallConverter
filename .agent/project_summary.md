...
**Status:** ✅ **v0.64 plugin / v0.51 pipeline (WIP)** — Dynamic redirect system is stable and fully working. IL2CPP hooks **definitively dead** (Exp 127). **Experiment 128/129 (2026-07-15):** `--enable-plugin`/`--disable-plugin` CLI flags **tested and verified LIVE on PS4** — both toggle correctly. BeatmapLevelSO blob builder produces correct IL2CPP-compatible serialized format **verified byte-for-byte against StartMeUp pack bundle** (exact field layout mapped). set_raw_data() via typetree FAILS for BeatmapLevelSO injection (IL2CPP PPtr mismatch). Generated test blobs for Espresso/Duvet/TimeLapse saved to `/workspace/beat_saber_deluxe/_beatmap_level_so_*.blob`. **TODO:** Raw SerializedFile manipulation or UnityPy type registry extension needed for actual CAB file injection. Plugin toggle is 100% complete and tested.
