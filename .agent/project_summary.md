...
**Status:** 🔴 **v0.65 plugin / v0.52 pipeline** — Mode selector attempt yielding CE-34878-0 crashes. All bundle-patching approaches (save("original"), manual bundle building, cab.save()+injection) crash the game at startup when the pack bundle is redirected. Root cause narrowed to either (a) UnityPy's CAB serialization format differs from original in ways PS4 Unity rejects, or (b) the 5-mode BeatmapLevelSO data references external BeatmapCharacteristicSO pathIDs that can't be resolved at load time. **Exp 133 (2026-07-15):** Identified and fixed two bugs — m_Script PPtr in blob builder (was using Standard char pathID instead of MonoScript pathID) and pipeline `--enable-plugin` not handling `;` as comment. Neither fix resolved the crash. **Current blocker:** Pack bundle patching via any UnityPy-involved approach crashes. Diagnostic test planned (Exp 134): deploy ORIGINAL unmodified pack bundle via redirect to test if redirect mechanism itself is compatible with pack bundles. **What works:** Per-song bundle redirect (Start Me Up → Espresso via `BeatmapLevelsData/startmeup` → `startmeup_v3`). Plugin toggle (`--enable-plugin`/`--disable-plugin`). Pipeline song building with PCM16 audio.
