...
**Status:** 🟡 **EXP 195: stale pack redirect bug ROOT-CAUSED + FIXED (pipeline v0.5324).** The 4-pack crash was caused by `_ensure_pack_bundle_redirects()` never removing stale pack redirects — so removing a pack from `pack_modes.packs` left its old redirect pointing at a patched bundle whose catalog entry no longer had a matching CRC. Fixed with hash-based stale removal. **1-pack config deployed** (therollingstones only, 40 redirects). 🔴 **AWAITING PS4 BOOT TEST** — user's PS4 was offline. Next: user boots → if clean → add packs back one at a time to isolate. Plugin stays at v0.8040. Also discovered: `_regenerate_merged_catalog` ignores `--pack-modes-packs` flag (always uses all configured packs).
