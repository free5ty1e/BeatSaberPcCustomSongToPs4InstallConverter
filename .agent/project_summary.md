...
**Status:** 🔄 **v0.59 plugin / v0.50 pipeline** — Dynamic redirect working. Mode selector: **ms_abi IL2CPP hooks deployed** (Exp 123). All three IL2CPP hook functions now use `__attribute__((ms_abi))` to match the MS x64 calling convention. `get_preview_detour` reads field at offset 0x98 directly and augments 1→3 preview sets. `set_content_detour` re-added to hook song selection. Mode selector: **SetData hook deployed** (Exp 120). Hooks `BeatmapCharacteristicSegmentedControlController.SetData()` to inject OneSaber/90Degree into the mode selector UI. Per-song bundle confirmed to have OneSaber/90Degree difficulty data (5 diffs each). Log shows previous get_preview hook was never called (field accessed directly via IL2CPP offset). New SetData hook intercepts the ACTUAL UI population point.
