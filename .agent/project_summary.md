...
**Current Status:** 🎯 v0.49 — VORBIS V4 deployed. vgmstream installed + verified original audio is "Custom Vorbis" (FSB5 Vorbis). Correctly decoded WAV saved. FSB5 format: size-prefixed raw Vorbis packets. Awaiting PS4 test. (Exp 88). Discovery: original audio IS HEVAG, fsb5 module's format interpretation was WRONG for PS4 FSB5. Hash field (16 bytes at offset 36) may be an FMOD content hash. Zeroing it to bypass validation. Awaiting PS4 test. with correct OGG headers. Pipeline integrated (--vorbis flag). Previous test confirmed game loads environment but audio was rejected (wrong VorbisData headers). Awaiting PS4 test.! Original audio is VORBIS (mode=15), NOT HEVAG! Vorbis FSB5 built (30s custom OGG). Bundle ready for PS4 test. Decoded PCM WAV saved for inspection.: Our HEVAG encoder is BROKEN (decode->encode->decode produces DIFFERENT output). PCM FSB5 test deployed (byte 8=0, 15s 440Hz sine). Researching alternatives: PS4 HEVAG coefficients, PCM path. If so, our encoder is valid and metadata updates are the root cause. PRX v0.49 deployed. Toolchain path persisted. Awaiting PS4 test.
