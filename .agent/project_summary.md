...
**Status:** 🟢 **EXP 212 (v0.5331): Self-Contained Single-Song `--deploy-full` + EXP 213: Pre-Deploy PS4 State Validation.** Fixed "it deploys all the other packs": single-song deploys now resolve the target's pack (`_resolve_target_pack`) and scope pack-mode bundles + catalog + `plugins.ini` + plugin + features.json + redirects to just that one song + its pack. `--deploy-full` implies `--deploy-plugin` + `--deploy-features`. New `development/scripts/ps4_state.py` inspects current PS4 BSD state (plugins.ini, bundles, configs, plugin) and prints a clean-slate/installed conclusion; wired into the PRE-DEPLOY CHECK of every `example_*` music-pack doc (.sh reads then "Press Enter to continue", .md documents it). 578 tests + live PS4 validation pass. Added `--deploy-full` to `full_custom_song_pipeline.py` that handles complete end-to-end orchestration in one command. All 5 music pack docs/scripts updated to use `--deploy-full`. Pipeline v0.5328 fully operational: 571/571 tests pass. **Backup utility script created with full plugins.ini support** — backup/clean/restore/list commands for PS4 GoldHEN filesystem via FTP. All 4 exercises (Restore from original backup, Backup without clear, Backup with clear, Restore from latest backup) fully succeed with actual PS4 content transfer. Script now backs up plugins.ini, safely removes BSD plugin entry during clean, restores plugins.ini, and validates plugins.ini state.
