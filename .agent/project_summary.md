...
**Status:** ✅ **REDIRECT NAMING BUG FIXED + DEPLOYED (2026-08-13, v0.5318)** — Discovered that the Aug-13 redeploy's `redirects.json` values pointed at stale `_v3` filenames (titlecase, no `.bundle`) while the 38 fresh builds deployed as `<slot>_v3.bundle` (lowercase). The plugin opens the redirect VALUE verbatim (case-sensitive) → game would silently serve stale builds. Pipeline now has a single source of truth (`_deployed_bundle_name`: canonical slot casing + `_v3.bundle` suffix) + `_ensure_mass_song_redirects` heals values on every config save. Corrected `redirects.json` (40 redirects) **deployed + verified on PS4**; **PS4 cleaned** (58 stale files removed, AFR dir 98→44 files). 419/419 tests. **AWAITING USER BOOT TEST** — all 38 songs should now load the NEW `.bundle` builds (Exp 184-186 deploy was otherwise verified).
...
8. **Integrate the pack-patch build script into the production pipeline** (`tools/`) — currently a dev script per rules.
