...
**Status:** 🟡 **EXP 199: v0.5325 "pathID fix" was ITSELF the boot crash - REVERTED in pipeline v0.5326, golden structure reproduced byte-identical, redeployed via pipeline. AWAITING USER BOOT TEST.** Hardware forensics: the RS bundle the user played all 4 modes on uses DISTINCT pathIDs per preview set (CHAR_PATH_IDS[mode]; nonexistent-in-pack pathIDs are safe), while v0.5325 pointed all 4 sets at Standard's pathID and that 4x-identical-PPtr structure CE-34878-0s at menu init. v0.5326 reverts that line, keeps rank-dedup padding; lizzo rebuild byte-identical to known-good (345d6a0e...), RS rebuild identical to golden (5ed23829...); 562/562 tests incl. new structure regression test; reference bundle archived in development/reference_bundles/. Deployed via --deploy-pack-modes --deploy-config --verify-ps4 (lizzo-only config), validation PASSED. User directive recorded: everything through the pipeline, fixes in pipeline code, no manual file manipulation.
