...
**Status:** 🔄 **v0.60 plugin / v0.50 pipeline** — Dynamic redirect working. Mode selector: **`get_preview_detour` with default C convention** (Exp 124). PS4 IL2CPP uses SysV AMD64 (confirmed by crash testing). All `ms_abi` removed. Only `get_preview` hook installed — reads field at offset 0x98 directly, augments 1→3 preview sets. Other hooks removed (set_data never fires, set_content caused crash). Mode selector: **SetData hook deployed** (Exp 120). Hooks `BeatmapCharacteristicSegmentedControlController.SetData()` to inject OneSaber/90Degree into the mode selector UI. Per-song bundle confirmed to have OneSaber/90Degree difficulty data (5 diffs each). Log shows previous get_preview hook was never called (field accessed directly via IL2CPP offset). New SetData hook intercepts the ACTUAL UI population point.
