...
**Status:** ✅ **EXP 191 (v0.5322): "STILL crashes" ROOT CAUSED — THE FIXED CATALOG WAS NEVER DEPLOYED. NOW DEPLOYED + VERIFIED + VERIFY HARDENED (2026-08-16)** — after the Exp 189/190 fixes, the user booted and STILL crashed. Fresh `bs_log.txt` showed the SAME signature (crash right after `aa/catalog.json -> REDIRECTED` at OPEN #58/#74). Downloaded the deployed catalog: md5 `0eb8a27d…` (795,783 B) = the OLD BROKEN v0.5319 build (70/2251 invalid dataIndexes), NOT the local fixed `975bacca…` (0 invalid). The Exp 189/190 pipeline fixes were correct but **the catalog was never redeployed** — and size-only verification couldn't catch it (broken and fixed catalogs are byte-identical in size). **FIXED:** (1) cleaned stale prototype files off the PS4 (`catalog_startmeup_modes.json`, `startmeup_pack_modes.bundle`), (2) deployed the fixed catalog (md5 `975bacca…`, 0 invalid, all 4 pack CRCs/sizes verified on-device), (3) **hardened `--verify-ps4` with check #7** — downloads the deployed catalog and validates dataIndex integrity + md5 match + per-pack CRC/size (a stale catalog now fails loudly), via new `validate_catalog_dataindexes()`/`validate_catalog_entries()`/`find_catalog_entry_js()` in `build_pack_mode_bundles.py`, (4) removed the legacy `pack_bundle` single-pack prototype from the DEFAULT config (superseded by `pack_modes`). 451/451 tests. **AWAITING: USER BOOT TEST** (boot should now be stable through the catalog redirect + pack scan).
