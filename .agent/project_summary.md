...
**Status:** 🔴 **v0.65 plugin / v0.52 pipeline** — All approaches to modify the Rolling Stones pack bundle crash at startup with CE-34878-0. Root cause ISOLATED: rebuilt bundles must use LZ4HC (flag=3) for ALL blocks, but even LZ4HC-rebuilt bundles need testing (Exp 135 just deployed). Additional blockers: UnityPy `cab.save()` produces incompatible CAB format (+4 bytes vs original), and UnityPy `save_typetree()` ignores modifications for BeatmapLevelSO in Unity 2022.3. Current approach: byte-level text patching of original CAB + bundle rebuild with LZ4HC flag=3 (text-only, blob size unchanged). **Exp 134a (2026-07-15) — ✅ Diagnostic PASSED:** Original pack bundle via redirect loads without crash, confirming redirect mechanism IS compatible with pack bundles. **Exp 134b (2026-07-15) — ❌ CRASHED:** Text-only patched bundle (LZ4 flag=2 blocks) crashed with CE-34878-0 at startup. Root cause: LZ4 flag=2 vs LZ4HC flag=3 mismatch. **Exp 135 (2026-07-15) — ⏳ AWAITING TEST:** LZ4HC-rebuilt bundle (flag=3, text-only patches) deployed via redirect. **What works:** Per-song bundle redirect (Start Me Up → Espresso). Plugin toggle. Pipeline song building with PCM16 audio. **Active blocker:** If LZ4HC rebuild also crashes, no viable approach currently exists for pack bundle modification.
