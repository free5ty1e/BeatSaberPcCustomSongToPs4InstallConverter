...
**Status:** 🔄 **v0.61 plugin / v0.50 pipeline** — Dynamic redirect + pack bundle patching (Exp 125). Mode selector: **pack bundle redirect** with modified BeatmapLevelSO preview data. The rollingstones pack bundle is redirected to a modified copy with OneSaber, NoArrows, 90Degree, and 360Degree added to `_previewDifficultyBeatmapSets`. Binary patching via `tools/patch_pack_bundle.py` uses `set_raw_data()` to work around UnityPy's save limitations. Mode selector: **SetData hook deployed** (Exp 120). Hooks `BeatmapCharacteristicSegmentedControlController.SetData()` to inject OneSaber/90Degree into the mode selector UI. Per-song bundle confirmed to have OneSaber/90Degree difficulty data (5 diffs each). Log shows previous get_preview hook was never called (field accessed directly via IL2CPP offset). New SetData hook intercepts the ACTUAL UI population point.
