...
**Status:** ✅ **v0.65 plugin / v0.52 pipeline (WIP)** — Mode selector complete! Pack bundle patched with 5-mode preview data via UnityPy `save("original")`. **Exp 132 (2026-07-15):** Breakthrough — UnityPy's `bf.save("original")` produces valid UnityFS bundles, unlike `save_fs()` which misses header strings. Modified StartMeUp's BeatmapLevelSO to expose all 5 characteristics (Standard/OneSaber/NoArrows/90Degree/360Degree) in the song-select mode picker. **m_Script PPtr bug fixed:** `build_beatmap_levelso_blob()` was using Standard's pathID instead of the correct MonoScript pathID (2140275054477726686). Output bundle (`rollingstones_pack_patched.bundle`, 7.9MB) verified by UnityPy — all 5 modes confirmed. **Plugin toggle** 100% complete. **TODO:** Deploy patched bundle + redirects to PS4 AFR when PS4 powered on. The redirect key `therollingstones_pack_assets_all_a99482a8a3da9e991e5ae36f2fea209c` -> `rollingstones_pack_patched.bundle` is added to redirects.json.
