...
**Status:** 🟢 **EXP 204 (v0.5328): BeatSaver MAP IDs Corrected to Use CUSTOM Songs (Not Target DLC Songs).** User caught that Exp 203 had incorrectly used target DLC song IDs instead of the actual custom song replacements. Fixed all 47 songs across 5 packs (Rolling Stones: 13, Billie Eilish: 10, Lizzo: 9, Camelia/Chromeo: 6, Britney Spears: 11) with correct custom song BeatSaver MAP IDs. Each pipeline command now has detailed comment with full custom song metadata (name, artist, album, year, MAP_ID, link, genre, BPM, difficulties). Pipeline v0.5328 fully operational: V3 schema normalization (17 keys), zero-note Easy rescue, color/direction restoration (c alternates 0/1, d cycles 0-7), BPM timing fix (m preserved, b=0 explicit). 571/571 tests pass.
