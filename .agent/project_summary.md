...
**Status:** 🟢 **EXP 214 (v0.5333): Complete single-song `--deploy-full` end-to-end working.** Fixed two issues:
1. **Download guard ordering** (v0.5332): `--download-beat-saver-song` resolution moved before early-exit guards so `--deploy-full` reaches song path.
2. **Validation scoping** (v0.5333): `verify_ps4_deployment()` now accepts `packs` filter and threads it through `_get_remote_pack_paths`, `_get_pack_bundle_redirects`, `_get_pack_modes_entries` so validation scopes to exactly the deployed pack(s). Also fixed size-check priority for fresh `custom_songs/` builds.
Fresh clean-slate deploy verified: 1 custom song, 1 modified pack (billieeilish), 3 redirects, plugin + plugins.ini + features.json + song_metadata.json, validation PASSED. 581 tests pass. (Prior: Exp 212 v0.5331 scoped single-song deploy; Exp 213 pre-deploy `ps4_state.py`.)
