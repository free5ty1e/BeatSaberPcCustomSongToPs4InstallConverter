...
| 10 | **FSELF format** | Deployed `--lib` output (FSELF wrapper, SCE magic `4f 15 3d 1d`) instead of OELF signed ELF | 🔄 AWAITING TEST | — |
...
- **Current hypothesis:** The `create-fself` tool (even v1.3) produces SCE authentication data incompatible with this PS4 firmware (9.00) + GoldHEN (2.3/2.4b16.2) combination. Working plugins on the PS4 were built with a different toolchain version.
- **Next steps (remaining options):**
  1. ⏳ Test FSELF format (--lib output) — currently deployed, awaiting result
  2. Try building with create-fself tag v1.0 (older, before bugfixes)
  3. Use a completely different toolchain (PS4SDK, flatz tools)
  4. Extract signed ELF from working RB4DX fself and analyze differences
