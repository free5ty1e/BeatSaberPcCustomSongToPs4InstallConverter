...
**Status:** 🟡 **v0.65 plugin / v0.52 pipeline** — **Mode selector attempt yielded CE-34878-0 crashes, root cause found: UnityPy cab.save() produces incompatible CABs (+4 bytes vs original, format rejected by PS4 Unity).** UnityPy save_typetree() also ignores modifications for BeatmapLevelSO objects (TypeTree serializer doesn't write back properly). **New approach — raw byte-level patching of the original CAB:** (1) modify the BeatmapLevelSO blob by replacing string content at known byte offsets while keeping blob size unchanged (440 bytes), or (2) use manual blob builder (struct-packing) with object table update for larger blobs (1257 bytes with 5 modes). **Exp 134 (2026-07-15):** Text-only pack bundle patching deployed — changed song name to "Espresso" and artist to "Sabrina Carpenter" via byte-level string replacement in original 440-byte blob. Bundle format fixed (separate `f.write()` calls + explicit alignment, `while f.tell()%16` is unreliable with concatenated writes). **Bundle building code verified working — UnityPy validates the patched bundle.** Bundle deployed via redirect. **What works:** Per-song bundle redirect (Start Me Up → Espresso). Plugin toggle (`--enable-plugin`/`--disable-plugin`). Pipeline song building with PCM16 audio. **Text patching deployed (untested)** — Awaiting user to restart Beat Saber and check if song info displays correctly. **Next:** If text patching works, try 5-mode selector using manual blob builder with object table update.
