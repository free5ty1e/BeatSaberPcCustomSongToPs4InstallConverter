...
**Status:** ✅ **v0.64 plugin / v0.52 pipeline (WIP)** — Dynamic redirect system stable. IL2CPP hooks **definitively dead** (Exp 127). **Exp 128/129 (2026-07-15):** `--enable-plugin`/`--disable-plugin` flags **tested LIVE on PS4** ✅. **Exp 131 (2026-07-15):** BeatmapLevelSO CAB binary injection **complete and verified** — `inject_pack_bundle.py` generates patched CAB files with raw byte-level replacement at verified offset 79924, size delta handled by extending CAB. All three blobs (Espresso=1257B, Duvet=1222B, Time Lapse=1251B) verified byte-by-byte against StartMeUp structure: m_Name, _levelID, BPM(double), 5-mode _previewDifficultyBeatmapSets all match exactly. set_raw_data() typetree approach **dead** (Exp 116/129). **TODO:** Deploy patched CABs via AFR redirect or raw bundle patching when PS4 powered on. Plugin toggle 100% complete.
