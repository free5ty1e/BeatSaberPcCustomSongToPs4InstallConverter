...
**Status:** 🟢 **EXP 207 (v0.5328): Unified --deploy-full Flag for Complete Orchestration.** Added `--deploy-full` to `full_custom_song_pipeline.py` that handles complete end-to-end orchestration in one command: song bundle + pack mode bundles + merged catalog + redirects.json + post-deploy validation. All 5 music pack docs/scripts updated to use `--deploy-full`. User can now run a single command per song that handles everything. Pipeline v0.5328 fully operational: 571/571 tests pass.
