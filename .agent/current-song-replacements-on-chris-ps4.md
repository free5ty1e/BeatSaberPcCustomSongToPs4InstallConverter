...
> **Pipeline:** v0.5319 (safe defaults: PCM16 + no-pad full audio + beatmap mode mapping/generators + V2→V3)
...
> **Last config:** 2026-08-14 — `redirects.json` (43 redirects) regenerated by v0.5319 with the **generalized pack_modes redirects** (4 packs: therollingstones/billieeilish/lizzo/camellia) + shared merged catalog `catalog_pack_modes.json` → `aa/catalog.json`. Post-deploy validation PASSED. **AWAITING USER TEST** — boot + check the 4 packs' mode selectors (4 modes on Hard+).
> **Status:** ✅ **DEPLOYED (2026-08-12/14)** — all 38 bundles + pack_modes bundles + `redirects.json` (43 redirects) + `song_metadata.json` verified on PS4 (sizes match local builds exactly). Plugin v0.8040 confirmed. **AWAITING USER TEST** — spot-check Chromeo slots (all 6), new Billie songs (Oxytocin/NDA/ThereforeIAm), 90Degree mode, full audio lengths, and the 4 patched DLC packs' mode selectors.
> **Mode mapping:** every custom slot now carries **Standard 5/5 + OneSaber 5/5 + NoArrows 5/5 + 90Degree 5/5** (generated gap-fill modes). The 4 configured DLC packs additionally have all their BeatmapLevelSOs patched to expose all 4 preview modes.
