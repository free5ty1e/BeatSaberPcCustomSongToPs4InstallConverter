# Installing Custom Songs Over the Soundtrack Vol. 6 Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 5 songs
in the official Soundtrack Vol. 6 DLC music pack with custom community songs from BeatSaver.
...
```
Cathedral, HeavyWeight, LiftOff, PowerOfTheSaberBlade, TempoKatana
```
...
## Target: Soundtrack Vol. 6 Official DLC Pack

- **Pack key**: `ostvol6`
- **Pack bundle**: `ostvol6_pack_assets_all_6fd14657d1a76a45e73d60348476b84e.bundle`
- **5 songs** (each with 5 difficulties: Easy, Normal, Hard, Expert, ExpertPlus):
...
```bash
# 1. Cathedral → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Cathedral     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. Heavy Weight → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target HeavyWeight     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. Lift Off → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target LiftOff     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. Power of the Saber Blade → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target PowerOfTheSaberBlade     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 5. Tempo-Katana → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target TempoKatana     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Soundtrack Vol. 6 pack are defined in:
