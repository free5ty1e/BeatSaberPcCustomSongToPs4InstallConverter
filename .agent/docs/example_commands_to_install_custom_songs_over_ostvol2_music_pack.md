# Installing Custom Songs Over the Soundtrack Vol. 2 Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 5 songs
in the official Soundtrack Vol. 2 DLC music pack with custom community songs from BeatSaver.
...
```
BeThereForYou, Elixia, INeedYou, RumNBass, UnlimitedPower
```
...
## Target: Soundtrack Vol. 2 Official DLC Pack

- **Pack key**: `ostvol2`
- **Pack bundle**: `ostvol2_pack_assets_all_c5f14abbd3060a65ad9020ee2c7dfcc9.bundle`
...
```bash
# 1. Be There For You → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target BeThereForYou     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. Elixia → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Elixia     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. I Need You → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target INeedYou     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. Rum n' Bass → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target RumNBass     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 5. Unlimited Power → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target UnlimitedPower     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Soundtrack Vol. 2 pack are defined in:
