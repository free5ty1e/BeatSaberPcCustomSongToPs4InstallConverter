# Installing Custom Songs Over the Soundtrack Vol. 7 Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 5 songs
in the official Soundtrack Vol. 7 DLC music pack with custom community songs from BeatSaver.
...
```
Damage, Lustre, TheMaster, Untamed, WorldWideWeb
```
...
## Target: Soundtrack Vol. 7 Official DLC Pack

- **Pack key**: `ostvol7`
- **Pack bundle**: `ostvol7_pack_assets_all_12bc27285a5ab70ed55bc8669dc39096.bundle`
...
```bash
# 1. Damage → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Damage     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. Lustre → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Lustre     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. The Master → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target TheMaster     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. Untamed → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Untamed     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 5. World Wide Web → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target WorldWideWeb     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Soundtrack Vol. 7 pack are defined in:
