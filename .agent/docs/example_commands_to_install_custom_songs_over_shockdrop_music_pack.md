# Installing Custom Songs Over the Shock Drop Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 3 songs
in the official Shock Drop DLC music pack with custom community songs from BeatSaver.
...
## Target: Shock Drop Official DLC Pack

- **Pack key**: `shockdrop`
- **Pack bundle**: `shockdrop_pack_assets_all_053675c7a6d0be532b79e39e2fafdea1.bundle`
- **3 songs** (each with 5 difficulties: Easy, Normal, Hard, Expert, ExpertPlus):
...
```bash
# 1. Abracadabra → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Abracadabra     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. Houdini → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Houdini     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. Not Like Us → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target NotLikeUs     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Shock Drop pack are defined in:
