# Installing Custom Songs Over the Soundtrack Vol. 4 Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 4 songs
in the official Soundtrack Vol. 4 DLC music pack with custom community songs from BeatSaver.
...
```
IntoTheDream, ItTakesMe, LudicrousPlus, SpinEternally
```
...
## Target: Soundtrack Vol. 4 Official DLC Pack

- **Pack key**: `ostvol4`
- **Pack bundle**: `ostvol4_pack_assets_all_223981efa5bc43e83c3926f8d141b466.bundle`
- **4 songs** (each with 5 difficulties: Easy, Normal, Hard, Expert, ExpertPlus):
...
```bash
# 1. Into the Dream → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target IntoTheDream     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. It Takes Me → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target ItTakesMe     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. LUDICROUS+ → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target LudicrousPlus     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. Spin Eternally → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target SpinEternally     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Soundtrack Vol. 4 pack are defined in:
