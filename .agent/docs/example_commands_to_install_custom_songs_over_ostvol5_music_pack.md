# Installing Custom Songs Over the Soundtrack Vol. 5 Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 6 songs
in the official Soundtrack Vol. 5 DLC music pack with custom community songs from BeatSaver.
...
```
CurtainsAllNightLong, DollarSeventyEight, FinalBossChan, Firestarter, IWannaBeAMachine, Magic
```
...
## Target: Soundtrack Vol. 5 Official DLC Pack

- **Pack key**: `ostvol5`
- **Pack bundle**: `ostvol5_pack_assets_all_c328930031222b8bdb2fb492b04cff21.bundle`
...
```bash
# 1. Curtains (All Night Long) → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target CurtainsAllNightLong     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. $1.78 → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target DollarSeventyEight     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. Final-Boss-Chan → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target FinalBossChan     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. Firestarter → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Firestarter     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 5. I Wanna Be a Machine → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target IWannaBeAMachine     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 6. Magic → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Magic     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Soundtrack Vol. 5 pack are defined in:
