# Installing Custom Songs Over the Timbaland Music Pack

This document provides step-by-step, self-contained pipeline commands to replace all 5 songs
in the official Timbaland DLC music pack with custom community songs from BeatSaver.
...
```
DumbThingz, Famous, HasAMeaning, WhatILike, WhileWereYoung
```
...
## Target: Timbaland Official DLC Pack

- **Pack key**: `timbaland`
- **Pack bundle**: `timbaland_pack_assets_all_2ce28c5b4ffb9523df0ccf229b092135.bundle`
...
```bash
# 1. Dumb Thingz → [YOUR CUSTOM SONG HERE]
...
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target DumbThingz     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 2. Famous → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target Famous     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 3. Has A Meaning → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target HasAMeaning     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 4. What I Like → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target WhatILike     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```

```bash
# 5. While We're Young → [YOUR CUSTOM SONG HERE]
# Custom Song: [Song Name]
# Artist: [Artist Name]
# Album: [Album Name]
# Year: [Year]
...
# BPM: [BPM]
# Difficulties: 5/5 (Easy through Expert+)
python3 tools/full_custom_song_pipeline.py     --download-beat-saver-song REPLACE_MAP_ID     --target WhileWereYoung     --pcm16     --no-pad     --convert-to-v3     --deploy-full
```
...
The song slot IDs and difficulty modes for the Timbaland pack are defined in:
