...
- **User test result (2026-08-07):** Boot clean — plugin notification v0.8040 shown, `startmeup` redirected (OPEN #763/#764), song loaded with no crash. **Selector still Standard-only — no non-standard modes to choose.** This matches the expected outcome exactly: the selector reads pack-level `_previewDifficultyBeatmapSets`, which remains unpatched; per-song bundles correctly carry the generated modes but the game's menu does not expose them.
- **Key discovery during log analysis:** the log shows `[OPEN #58] /app0/Media/StreamingAssets/aa/catalog.json` passing through `open_hook`. This **contradicts** the Exp 136 KB claim that "catalog.json cannot be redirected" (that claim was based on the OLD plugin hook model — AssetBundle::LoadFromFile only). v0.8040 hooks libc `open()` (GoldHEN Detour), and `open_hook` substring-matches paths against `LOWER_REDIRECT_KEYS`, so a key containing `aa/catalog.json` could now redirect it. **This re-opens the catalog-redirect avenue** to relax the Addressables catalog CRC+size dual validation for a patched pack bundle. Log archived to `experiment_logs/v0.8040_pipeline0.5310_exp177_startmeup_modes_test.txt`; `bs_log.txt` cleared.
- **Status:** ✅ Deployed + user-tested. Generators/fill-in pipeline validated end-to-end (14 mode files on disk, no boot issues). Selector gap **confirmed** — next work is the catalog-redirect / pack-patch avenue (see Exp 178+).

### Experiment 178: Catalog-Redirect Feasibility Proof — byte-identical `catalog_test.json` (AWAITING BOOT TEST)
- **Date:** 2026-08-07
- **What:** First step of the catalog-redirect avenue: prove the `aa/catalog.json` redirect key actually intercepts the catalog open (OPEN #58) with ZERO risk. Copied the original catalog byte-for-byte to `catalog_test.json` (793,186 B, md5 `dc6e9303e0227818bdaa8b397a350c89`) and appended `"aa/catalog.json": "catalog_test.json"` to `redirects.json` (now 39 entries, 2,290 B). Uploaded `catalog_test.json` to `/data/GoldHEN/AFR/CUSA12878/catalog_test.json` and `redirects.json` to the same dir; both verified present on-device; `bs_log.txt` cleared for a clean session.
- **Result:** 🔲 **DEPLOYED + VERIFIED ON-DEVICE — AWAITING USER BOOT TEST.** On boot report, pull `bs_log.txt` and confirm line 83 area shows `[OPEN #58] /app0/Media/StreamingAssets/aa/catalog.json -> REDIRECTED` and that boot is stable.
- **Key design points:** The catalog bytes are IDENTICAL to original (so even if Addressables re-validates catalog integrity, nothing changes except the redirect). Bundle paths inside the catalog are absolute (`{RuntimePath}/PS4/...`) so serving the catalog from AFR does not break bundle resolution. `redirects.json` is read at open #4, well before catalog open #58, so the redirect is armed in time.
- **Version:** Plugin v0.8040 / Pipeline v0.5310 (unchanged — no code change)
- **Status:** 🔲 Deployed, awaiting boot confirmation. If stable → Exp 179 deploys the patched pack bundle + matching fresh catalog.

### Experiment 179: Pack-Bundle Preview-Set Injection — `startmeup_pack_modes.bundle` + fresh catalog (BUILT + VERIFIED)
- **Date:** 2026-08-07
- **What:** Built the actual pack-bundle patch that closes the mode-selector gap, using the catalog-redirect avenue to satisfy the Addressables CRC+size dual validation.
  - **CRC root cause (SOLVED):** catalog `m_Crc` = `zlib.crc32` over the **DECOMPRESSED stream** of the bundle, NOT the compressed file. Verified: original bundle file zlib.crc32 = `0x63520032`, but original catalog `m_Crc` = 3700109647 = `0xdc8b314f` = zlib.crc32 of its 8,511,228-byte decompressed stream. This explains why the Exp 142–157 "CRC corrected" bundles still crashed (they fixed the file CRC, not the decompressed-stream CRC) and makes GF(2) alignment-padding CRC forcing unnecessary — we simply write the patched bundle's ACTUAL decompressed-stream CRC + file size into the fresh catalog.
  - **Patch (surgical):** took the original 440-byte StartMeUp BeatmapLevelSO blob (object `2287600824654271910`) and appended OneSaber/NoArrows/90Degree preview sets (each 5 difficulties, difficulty data copied verbatim from Standard). Blob 440 → 1,028 B. Identity fields (`_levelID`="StartMeUp", `_songName`, `_previewAudioClip`, `_coverImage`, environment, `_contentRating`=1) preserved byte-for-byte so the levelID→BeatmapLevelsData per-song redirect still fires. Object table offsets for objects after the blob updated (only 1 subsequent song object shifted 85,248 → 85,836; all data intact). CAB rebuilt with LZ4HC; 81/81 objects parse; UnityPy typetree reads correctly on the patched bundle (songName "Start Me Up", levelID "StartMeUp").
  - **Fresh catalog:** `catalog_startmeup_modes.json` — single-entry diff from original: rolling-stones pack `m_Crc` 3700109647→**2384431415** (=0x8e1f8937, the patched bundle's decompressed-stream CRC), `m_BundleSize` 7902803→**7905425** (patched file size). `m_Hash` `a99482a8...` and `m_BundleName` `51dc790300eb3d900786837beb3ac335` unchanged. UTF-16LE extra-data string preserved exactly (byte-length identical).
  - **Build tool:** `development/scripts/build_startmeup_pack_modes.py` (294 lines; dev-script stage per rules, not yet integrated into the production pipeline). Outputs `startmeup_pack_modes.bundle` (7,905,425 B) and `catalog_startmeup_modes.json`.
...
### Experiment 183: Full-Audio Deploy Prep + Pipeline Idempotency Fix (2026-08-11)
- **Date:** 2026-08-11
- **Context:** v0.5314 baked safe defaults into the pipeline (PCM16 + no-pad + mode mapping + V2→V3 all default ON, each with oppose flags; `--features-only` mode added; mode-mapping flag removed from runtime `features.json`). 404/404 tests. This session: verify the new default command reproduces the verified full-audio bundle, fix what breaks it, and deploy to PS4.
- **Idempotency bug found + fixed (pipeline v0.5314→0.5315):** Re-running the pipeline on a source dir that already contains generated mode `.dat` files (from a prior run or hand-authored) produced a *different, degraded* bundle: `apply_mode_mapping()` gated TextAsset injection on `song_dir AND non-empty generated_files`, but `add_mode_characteristics()` already scans `song_dir` for ALL mode files (pre-existing included). With `generated` empty (all modes already on disk), injection silently fell back to cloning Standard references — mode selectors appeared but played Standard data. **Fix:** gate now only requires `song_dir`. Regression test `test_idempotent_injection_with_pre_existing_mode_files`. 405/405 tests pass.
- **Stale 90Degree source files regenerated:** the candidate source dir's `Easy/Normal/Hard/ExpertPlus90Degree.dat` (generated Aug 6 by v0.5310-12 code) were already V3 with the OLD `t` event-type field (lighting deserializes as type 0). Regenerated in-place with `_generate_90_degree(bpm=130)` (song BPM, matches Info.dat). Now byte/semantic-identical to the verified bundle's injected 90Degree TextAssets. Note: `90DegreeExpert.dat` is the song's own V2 file — converted to V3 at injection (expected diff, correct).
- **Rebuild verified:** fresh `startmeup_v3.bundle` (39,697,727 B) built with the new default command (`--song-dir <drop_pop_candy dir> --target startmeup`, no flags). All 22 TextAssets semantic-identical to the previously-verified 39,697,733 B bundle (6-byte UnityFS blocksInfo header diff only — compression block split, no content difference). AudioClip = full 224.31s PCM16, 39,568,005 B resource.
- **Deploy status:** 🔴 **BLOCKED** — PS4 FTP (192.168.100.117:2121) flaky/down this session (intermittent ls success, persistent get/put timeouts). Verified bundle rebuilt & ready at `/workspace/startmeup_v3.bundle`. features.json (2 runtime flags) ready.
- **Version:** Pipeline v0.5315
- **Status:** ✅ Idempotency fixed + verified reproducible default build. 🔴 PS4 deploy pending (FTP down). After deploy: user boot test (full 224.31s audio, 4 mode selectors, source 90° Expert lane behavior, lighting `et` events).
- **Next steps:** retry FTP → deploy `startmeup_v3` + `features.json` → clear `bs_log.txt` → user boot test.
