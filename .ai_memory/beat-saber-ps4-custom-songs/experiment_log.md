...
### Experiment 226 — Mode Buttons Only on First Pack (Scoping) + clear-target-song Redirect Wipe + Missing/Off-Target Songs
- **Date:** 2026-09-23
- **User report (after running all 5 pack scripts, all succeeded):** (1) only billie eilish songs show mode-selector buttons; every other pack shows stock modes; (2) BE slots IDidntChangeMyNumber/Oxytocin/YouShouldSeeMeInACrown and RS StartMeUp were never replaced (absent from the scripts); (3) Toxic/Radar are Britney songs installed in the britney pack — replace with non-target-artist songs.

**Root cause 1 — pure-filter pack scoping (the mode buttons bug).** PS4 log (archived `v0.5343_multipack_modes_missing.txt`): final boot `breakdown: 44 songs, 1 packs, 1 catalog` — only billieeilish_pack ever REDIRECTED. Each per-song deploy passed `deploy_packs` through helpers that FILTERED the active set (`[p for p in active if p in packs]`), where `active` = deployed-state discovery — which lags the pack being deployed right now. The britney deploy found only billieeilish active → `Requested pack(s) not in the active set — nothing to deploy` → britney pack patch silently skipped (validation then passed against the 1-pack state, so nothing flagged it). **Fix: union semantics in all four helpers** (`_get_pack_modes_entries`, `_ensure_pack_mode_bundles`, `deploy_pack_bundle`, `deploy_pack_modes`): requested packs JOIN the active set; discovery adds, never removes. Rule recorded: deployed-state discovery must ADD to a deploy's scope, never gate it.

**Root cause 2 — clear-target-song redirect wipe (found live while repairing).** Clearing SugarSoaker (the misplaced PATD slot, see below) — as the last "custom" of its pack — passed `slots=rebuild_slots=[]` into redirect regeneration; `_ensure_mass_song_redirects`' empty-scope branch deleted ALL 44 song redirects from every pack. **Fixes:** clear_target_song's step-6 re-ensure scopes to the song redirects that still EXIST after its surgical removal; empty scope = NO-OP; unmatched non-empty scope preserves existing redirects; out-of-scope trimming only runs when the scope matched configured slots. Live state restored by rebuilding redirects.json from the deployed AFR bundles (44 songs + 5 pack pairs + catalog) — validation PASSED.

**Root cause 3 — misplaced PATD slot in the RS script.** `SugarSoaker` is a **Panic! At The Disco** slot (verified in song_ids.json), wrongly listed in the Rolling Stones script since the 30-scripts batch. The user's RS run deployed Venom of Venus over a PATD song and patched the PATD pack. Removed from RS script/docs (RS = 11 slots: Angry, BiteMyHeadOff, CantYouHearMeKnocking, GimmeShelter, ICantGetNoSatisfaction, LiveByTheSword, MessItUp, PaintItBlack, StartMeUp, SympathyForTheDevil, WholeWideWorld); PATD deployment reverted (slot cleared, pack redirect removed, catalog regenerated for the 5 intended packs).

**Song completions/swaps (all download-verified with clean note-count progressions; user directive: replacements must not be by the target artist):**
- BE: IDidntChangeMyNumber → **Mr. Brightside** (The Killers, 3fc90, 5/5); Oxytocin → **Kiss Me More** (Doja Cat ft. SZA, 4dea2, 5/5); YouShouldSeeMeInACrown → **15 Minutes** (Sabrina Carpenter, 443f3, 5/5)
- RS: StartMeUp → **Wake Me Up** (Avicii, 437d, 5/5)
- Britney: Toxic slot → **Complicated** (MELODICKA BROS Avril cover, 19849, 5/5); Womanizer slot → **Teenagers** (My Chemical Romance, 217f1, 5/5); OopsIDidItAgain slot → **Hollaback Girl** (Gwen Stefani, 53e4, 5/5) — replacing the DITR4 fan re-chart per the user's follow-up ("I don't care about fan re-charts"; no target-artist songs at all)
- Rejected candidate: Stereo Love 266cb (duplicate charts: Easy=Hard, Normal=Expert)

**Verification:** Mr. Brightside deployed (BE pack) — with union fix, ALL 5 packs built + deployed + catalog 5 entries; `--verify-ps4` PASSED (56 files, all targets, catalog md5 match, all 5 packs' pairs). Replacements mapping updated (RS table 12→11 slots + StartMeUp row; BE table 13 songs; new Britney section 11 songs). 3 wipe-safety regression tests + 2 scoping tests (union semantics). **Tests: 613/613.**
- **Status:** ✅ All fixed + live state repaired. **User next steps:** re-run the BE script (songs 1-10 redeploy idempotently; 11-13 are the new slots) and RS script (StartMeUp now included), then the britney script for the Toxic/Womanizer swaps; or pick up as-is — the mode buttons for all 5 packs are already live.

---

### Experiment 227 — Ghost Packs on Clean Slate (Exp 226 union over-corrected) + Clean Slate Must Wipe Local State
- **Date:** 2026-09-23
- **User report:** clean slate → installed first 3 BE songs → `--clear-target-song AllTheGoodGirlsGoToHell` → console processed/deployed britneyspears, camellia, lizzo, panicatthedisco, therollingstones — packs NEVER installed on the clean PS4. Follow-up directive: "the backup command with the clear parameter... should INCLUDE the local state too! Especially if anything can fallback on local caches."
- **Root cause 1 — the Exp 226 union used the wrong discovery source.** The union `configured = list(packs) + [p for p in _resolve_active_packs(config) if p not in packs]` added the LOCAL-BUNDLE FALLBACK set: on a clean slate (no redirects.json state), `_resolve_active_packs` falls back to "packs with local built bundles" — and `pack_modes_bundles/` still held 6 bundles from earlier repair sessions. The fallback exists for BUILD-time scope, but the deploy flows used it as "what's already deployed" → ghosts. **Fixes:** (1) new `_resolve_deployed_packs()` — deployed state (redirects.json) ONLY, no fallback, returns [] on clean slate — now drives the unions in `deploy_pack_bundle` + `deploy_pack_modes`; (2) caller-provided `packs=` is AUTHORITATIVE in `_get_pack_modes_entries`/`_ensure_pack_mode_bundles` (they honor it verbatim; deploy flows do their own unioning). Verified: clear-song scope = [billieeilish] only; single-song cross-pack preservation intact (britney deploy with BE+camellia deployed → scope = all three).
- **Root cause 2 — clean slate didn't clean local state (the user's directive).** `--clean-ps4` cleared only song_metadata.json/redirects.json/catalog_pack_modes.json locally; `pack_modes_bundles/` (ghost-pack source), `custom_songs/`, `mass_bundles/`, and stray `_beatmap_level_so_*.blob` files survived. **Fix:** `clear_local_pipeline_state()` now removes all three dirs + the blobs (verified live: 3 configs + 8+69+39 files + 51 blobs; all regenerable from BeatSaver + game dump).
- **Lesson (durable):** a fallback that exists for one flow (build-time scope) must never be consumed by a different flow (deploy-time preservation) — the deploy flows now have their own strict resolver. And a "clean slate" definition must cover EVERY local input that any code path can fall back to.
- **Test note:** the full-suite run after the local-state wipe initially failed 59 tests (they read the real bundles/configs my dry-run deleted) — artifacts rebuilt from sources via `_ensure_pack_mode_bundles` + PS4 state re-download, confirming full regenerability. Tests: **616/616** (3 new ghost-pack regressions; 5 scoping tests updated to authoritative semantics).
- **Status:** ✅ Fixed + verified. User can re-run: clean slate (now truly clean) → BE script → clear one song → only billieeilish touched.

---

### Experiment 228 — '…Baby One More Time' Unicode Metadata Miss + V4 Columnar Beatmaps (Found via Deployed-Bundle Audit)
- **Date:** 2026-09-24
- **User report:** all 5 pack scripts chained end-to-end succeeded, tests went well; ONLY issue: the Britney pack's "...Baby One More Time" slot title metadata not replaced with "Take On Me / A-ha". Suspected special-character string matching. Also asked about 3 files git status showed deleted (`custom_songs/fsb5_header_template.bin`, `quick_test.bundle`, `quick_test_gen.py`).
- **PS4 log (archived `v0.5345_multipack_bomt_metadata_title_miss.txt`, 1835 lines):** every Britney title replaced (`Circus`→Shape of You … `Womanizer`→Teenagers, MoveNext #47–#67) INCLUDING `Oops!...I Did It Again` (ASCII dots — matched fine) — but the BOMT cell fired ONLY the author replacement (#47 'Britney Spears'→' '); the songName lookup returned nothing. Root-cause chain confirmed on-disk: the PS4's `song_metadata.json` carried the key as the escaped ASCII sequence `…Baby One More Time` (Python `json.dump` default `ensure_ascii=True`), while the game's UTF-16 title extracts to `?Baby One More Time` (`extract_utf16_string` folds any non-ASCII code unit to '?'). Three independent bugs: (1) pipeline wrote `\uXXXX` escapes, (2) plugin's byte-verbatim `parse_json_pairs` kept them, (3) the fold mismatch means even unescaped raw UTF-8 wouldn't have matched.
- **Fixes (pipeline v0.5346 / plugin v0.8047):**
  1. Pipeline writes `song_metadata.json` raw UTF-8 (`ensure_ascii=False`, all 3 dump sites) — human-readable, plugin-parse-friendly.
  2. Plugin `json_unescape_inplace()`: `\uXXXX`→UTF-8 + standard escapes, applied after pair parse (backward compat: the already-deployed escaped file would ALSO have matched without redeploying).
  3. Plugin `fold_utf8_to_ascii()`: folds loaded KEYS through the same projection as extraction (BMP → 1 '?', astral → 2 '?' — exact mirror of two UTF-16 units); values stay raw UTF-8 for display.
  4. Plugin `create_il2cpp_string` manual fallback now decodes UTF-8→UTF-16LE (was byte-per-codeunit mojibake); `il2cpp_string_new` (primary) already handled UTF-8.
- **Second live bug found during RCA — V4 columnar beatmaps + a corrected initial analysis.** The deployed `Oxytocin_custom.bundle`/`YouShouldSeeMeInACrown_custom.bundle` contained beatmaps with `colorNotesData` — BeatSaver v4.0.x columnar format. FIRST (wrong) read: "notes with only {b,i} → stacked at x=0/y=0, charts broken". CORRECTED after checking the stock dump CAB (`beatsaber_song_exports/CAB-0183cf...`): the PS4's NATIVE format is the SAME columnar layout (`version: 4.0.0`, 116 notes → 19 deduplicated `colorNotesData` rows, `i` defaults 0) — the game parses columnar natively, which is why the user's Standard tests on these songs played fine. The REAL hazard: the procedural mode generators mutate `colorNotes` events assuming denormalized fields — on columnar events they attach inline `c`/`d` ALONGSIDE the `i` index → ambiguous mixed events for the generated OneSaber/NoArrows/90Degree charts. Also: V4 Info.dat uses `song.title`/`song.author`/`audio.bpm` (not `_songName`/`_songAuthorName`/`_beatsPerMinute`), so the display name silently fell back to the map ID (`'Oxytocin' -> '4dea2'` in the deployed metadata). **Fixes:** `is_v4_beatmap()` + `convert_v4_to_v3()` denormalizer (all columnar pairs: colorNotes/bombNotes/obstacles/arcs/chains/basicBeatmapEvents/colorBoost) wired into `replace_beatmaps` + both mode-injection donor paths — everything normalized to one canonical denormalized layout so the generators operate on unambiguous data; `_read_info_song_metadata()` + `_read_info_bpm_from_dict()` read V4 keys at both Info.dat sites; `download_beat_saver_song` warns loudly on map-ID fallback; both live bundles REGENERATED + REDEPLOYED (verified: v3.2.0, full x/y/c/d notes, no `colorNotesData` residue) and their metadata entries corrected via the new `--metadata-only` mode ('Kiss Me More / Doja Cat', '15 Minutes / Sabrina Carpenter').
- **New CLI:** `--metadata-only` — surgical add/update/deploy of a `song_metadata.json` entry with no song processing/redirects/plugin (the Exp 199-compliant path for metadata repairs; used twice live).
- **Deleted-files question:** the 3 git-tracked files were casualties of the Exp 227 `--clean-ps4` wipe (it `rmtree`d `custom_songs/` wholesale). Restored via `git restore`; `fsb5_header_template.bin` is a LIVE production dependency (`tools/hevag_encoder.py` DEFAULT_FSB5_TEMPLATE — legacy `--hevag` codec path; PCM16 default builds headers from scratch, which is why deploys still worked). `quick_test*` are old dev fixtures referenced by `tests/analyze_audio.py` docs. **Follow-up:** make the wipe skip git-tracked files (Exp 229 candidate) so this class of accidental deletion can't recur.
- **Tests:** 18 new (10 Unicode metadata + fold mirrors, 8 V4 conversion incl. real-4dea2 shapes); **634/634 pass**.
- **Status:** ✅ Fixed, deployed live (plugin v0.8047 + corrected metadata + both v4 bundles), staged. **User test:** boot the game — BOMT should now show "Take On Me / A-ha"; Oxytocin/Crown titles show real names; their Standard/OneSaber/NoArrows/90Degree charts now have real note placement.

---

### Experiment 229 — CI Failure RCA + Release Artifact Audit + True Clean-Slate Wipe Safety
- **Date:** 2026-09-24
- **User report:** all 5 pack scripts re-verified from clean slate — release focus. Asked to (1) audit CI/release pipeline for complete release artifacts (all example_*.* files must ship), with release notes stating the user supplies their own decrypted dump; (2) fix the failing GitHub PR CI via gh; (3) update the very-out-of-date PR title/description to the branch's expanded scope.
- **CI failure RCA (gh read-only):** build job = 214 test failures; Lint job = 17 ruff errors; `release` job skipped (gated on build); `Unit Tests` job showed GREEN with 107 failures inside — `2>&1 | tee` swallowed pytest's exit code.
  - *Test failures, cause 1 — absolute paths:* test files hardcoded `/workspace/beat_saber_deluxe/...` and `/workspace/ps4_dump/...`; GitHub runners check the repo out at a different path → `_load_pack_albums` found no song_ids.json → entries=[] → TestRequestedPackUnion asserts failed with EXACTLY the CI diff shape (`[] != ['billieeilish']`). All paths now derived from `__file__`.
  - *Test failures, cause 2 — hardware-environment tests with no skip guards:* `test_pack_mode_pipeline.py` (115 tests over 9 classes) hard-depends on the decrypted game dump + locally-built patched bundles — impossible on a runner. Added `HAS_DUMP`/`_HAS_BUILT_ALL` module gates + per-class `pytestmark` (all four fixture packs required so mid-deploy partial builds skip, not fail).
  - *Lint:* 13 auto-fixed (F401/F541/I001) + 4 hand-fixed F841s (dead `changed` flags in `normalize_v3_schema` — the flag was vestigial; the function documents in-place mutation).
  - *ci.yml pipefail:* `set -o pipefail` added so the green Unit Tests check can never lie again.
- **Live-event interplay (important):** the user's clean-slate retest was RUNNING during this work (BE → britney → … chained scripts). Two consequences handled: (a) their `--clean-ps4` run used the OLD buggy wipe and re-deleted the 3 git-tracked `custom_songs/` files (restored again); (b) my scratch-test of the new wipe accidentally ran against real state files (cleared the 3 regenerable local configs mid-deploy — the running pipeline rewrites them per song, so no corruption; noted for the user). **The wipe fix now enumerates `git ls-files` per state dir and preserves tracked files** (scratch-tree verified: tracked survives, artifacts removed, empty subdirs pruned).
- **Release artifact audit (plugin-build.yml + CI_RELEASE.md + README):** zip now ships plugin binaries (release+debug), plugins.ini, empty redirects.json template, features.json, ps4_config.example.json, both changelogs, VERSION, README + PIPELINE-README, 3 feature docs, pack how-to docs, **all 69 example_*.* under docs/example-scripts/**, beat_saber_song_ids.json (slot catalog), requirements.txt, coverage report, tools/ (minus __pycache__). **ps4_dump disclaimer added to CI_RELEASE.md (Prerequisites, bolded) + README prerequisites**: release contains NO game data; user must dump their own correct version (v2.04) + DLC loadout into ps4_dump/.
- **PR #4 updated via `gh api -X PATCH`** (the `gh pr edit` GraphQL path fails with the Projects-classic deprecation): new title "Beatmap Mode Mapping: 4-mode song selection for all custom songs + missing-difficulty auto-fill + production-grade pipeline (Exps 160-229)"; body rewritten to the true scope (mode mapping + generators + auto-fill, pipeline hardening, plugin flags/toast/Unicode, docs+examples, test/CI status, hardware verification).
...
### Experiment 231 — NoArrows Arrows on V4 Maps (15 Minutes) + The Recurring 3-File Deletion (cwd bug)
- **Date:** 2026-09-25
- **User report (post-Exp 230 QA pass):** spot check on 15 Minutes (Sabrina Carpenter, 443f3) Hard **NoArrows** showed ARROWS. User asked: if this is a swiss-cheese song with missing difficulties, shouldn't the auto-fill have given the generator source material? Also: the 3 `custom_songs/` files deleted AGAIN.
- **RCA 1 — not a missing-difficulty failure.** 443f3 provides all 5 difficulties natively (Easy/Normal/Hard/Expert `*Standard.dat` + ExpertPlus via `ExpertPlus.beatmap.dat` — v4 bare tier, resolved correctly by `_select_beatmap_file` tier 3). `fill_missing_standard_difficulties` = no-op (correct); `generate_missing_mode_beatmaps` generated all 15 mode files (correct); `gen_lookup` matched all 15 (correct). The failure was DOWNSTREAM: the deployed `HardNoArrows.beatmap.gz` was byte-identical to Standard (d-distribution 220/175/30/32/69/65/46 = arrows + 46 legit dots).
- **Root cause — v4 columnar clobber at injection.** BeatSaver v4 sources are columnar: `colorNotes` events `{b, i}` + `colorNotesData` rows `{x, y, c, d}`. The NoArrows generator set d=8 on the EVENTS (deep-copied dict keeps the rows); at injection `convert_v4_to_v3` merges **row-OVER-event** (`obj.update(row)`) — the rows' arrow d-values clobber the generator's dots. OneSaber was immune (post-merge re-application since Exp 218); 90Degree immune (generator only APPENDS rotationEvents — the row merge can't touch them; deployed 90Degree verified: 52 rotations on Hard90, 0 on Standard). **Fix:** the injection path re-applies `_generate_no_arrows` AFTER the v4/v2 conversion — idempotent for well-formed sources (d=8 stays 8), corrective for columnar. Deliberately NOT a blanket `_MODE_GENERATORS` re-application — 90Degree's rotation-append is NOT idempotent (would double rotations).
- **RCA 2 — the 3-file deletion recurred because the Exp 229 fix was cwd-dependent.** `_git_tracked_files` ran `git ls-files` inheriting the process cwd — from the repo root it works; from ANY other launch directory git resolves to no repository → returns [] → the wipe's else-branch rmtree'd `custom_songs/` wholesale. My scratch test passed only because it happened to run from /workspace; the user's real runs (launched elsewhere) didn't. **Fix:** `git -C <repo_root> ls-files` with the repo root derived from the script's own path (`Path(__file__).resolve().parent`). Verified from /workspace, /tmp, / — all three track the files now.
- **The 3 files (answered for the record):** all three ARE needed and tracked. `fsb5_header_template.bin` = live production dependency (`tools/hevag_encoder.py` DEFAULT_FSB5_TEMPLATE — the legacy `--hevag` codec's FSB5 sample-header template, fixed at 1732 bytes by Exp 76; the PCM16 default builds headers from scratch, which is why deploys kept working without it). `quick_test.bundle` + `quick_test_gen.py` = dev fixtures referenced by `tests/analyze_audio.py` / experiment archive notes.
- **Live repair:** both v4 songs affected (Oxytocin/4dea2 confirmed identical breakage — Easy/Normal d-values [1,2,3,8], Hard+ d-values [1-8]). Both REDEPLOYED with v0.5348; verified: all 10 NoArrows charts (5 per song) now d=[8] all-dots, geometry (x/y/c) preserved; PS4-side bundles + BE pack bundle + catalog live (17:26-17:30 timestamps).
- **Tests:** 5 new (TestV4NoArrowsClobberFix: the clobber documented, post-conversion pass, idempotency, source-audit of injection ordering + no-blanket-90Degree, tier-3 ExpertPlus selection). **639/639 pass**, ruff clean. VERSION → **0.5348**.
- **Status:** ✅ Fixed + deployed live. **User re-test:** play Hard NoArrows on 15 Minutes (and Oxytocin) — all dots expected.

---

### Experiment 232 — CRITICAL: Chained Multi-Pack Deploy on Clean Slate Keeps Only the LAST Pack
- **Date:** 2026-09-25
- **User report:** true clean-slate PS4 + the 5-script chain → only Rolling Stones customs work; the other 4 packs show swapped metadata (song_metadata.json accumulates fine) but stock beatmaps/modes/songs. User did NOT commit the Exp 231 changes pending this RCA.
- **PS4 log (archived `v0.5348_multipack_regression_only_rs_working.txt`):** final boot `13 redirects — 11 songs, 1 packs, 1 catalog`; britneyspears/camellia/lizzo/therollingstones pack bundles opened UNREDIRECTED (stock); all 5 patched pack bundles verified present on the PS4 AFR — the bundles were fine, only redirects.json lost 4 pack entries. Local redirects.json matched: 11 songs, 1 pack (RS), catalog.
- **Root cause:** `_ensure_pack_bundle_redirects`' stale-removal sweep validates "still configured" by the CURRENT pair — and `_get_pack_bundle_redirects` filters by LOCAL bundle existence (`os.path.isfile(e['local_path'])`). The Exp 227 clean-slate wipe empties `pack_modes_bundles/`; each script's deploy then has only ITS OWN pack's bundle locally → the other packs' redirects classify as "no longer configured" → DELETED. Chained: script N deletes script N-1's entry. Last writer (RS) wins. **Reproduced deterministically** (1→1→1→1→1 in the sim). **The bug is LATENT SINCE Exp 226 (v0.5344)** — present in HEAD (proven by stashing Exp 231's uncommitted pipeline change and re-running the chain sim: identical 1-pack final state). Prior chained QA runs passed ONLY because `pack_modes_bundles/` still held all 5 packs' bundles from earlier sessions, making the local-existence filter accidentally permissive. The Exp 227 true-clean-slate wipe (user directive) exposed it. Song redirects were immune: their union uses the PS4 state download, not local files.
- **Fix:** in the stale sweep, a pack redirect is VALID if in the current pair (local bundle present) OR the pack is DEPLOYED per the redirects state file (`_resolve_deployed_packs` — the same authority the deploy flows use, Exp 226/227 architecture). True stales (neither) still removed. Verified: clean-slate chain sim accumulates 1→2→3→4→5; deployed-pack-with-missing-local-bundle survives; ghost pack still removed.
- **Second-layer finding (self-heal deadlock):** after the bug corrupts redirects.json, `_resolve_active_packs` trusts the state file unconditionally ("state file is authoritative") → the repair flow itself couldn't re-add the packs (state says 1 pack → pair says 1 pack). Repaired live by calling `_ensure_pack_bundle_redirects` with the explicit 5-pack scope (all bundles verified live on PS4), then regenerating the catalog for all 5 + deploying. Also removed two long-ghost PATD song redirects (deadmanwalking/sugarsoaker — Exp 226 revert leftovers pointing at nonexistent bundles) via `--clear-target-song`. **Final live state: validation PASSED — 5 packs, 36 songs, catalog md5 match.**
- **Durable lesson:** a state-authority resolver and a file-existence filter make a broken pair — any sweep that DELETES deployed state must validate against the deployed-state authority, never against local cache existence. Local caches are build inputs; the state file is the record of what's live.
- **Tests:** 3 new (TestChainedPackRedirectAccumulation: clean-slate 5-chain accumulation, deployed-survives-missing-bundle, true-stale removal). Suite: 642 expected green (one transient failure was a race with the live repair rewriting redirects.json mid-suite — passes in isolation).
- **Status:** ✅ Root-caused, fixed, live state repaired, staged. VERSION → **0.5349**. User re-test: chained 5-script run from clean slate → all 5 packs must keep custom songs + 4 mode buttons. (Also validates Exp 231's NoArrows fix on the v4 songs.)

---

### Experiment 233 — The Real 3-File Deletion Bug (relative-vs-absolute comparison) + --no-prompt + Last CI Gates
- **Date:** 2026-09-25
- **User report:** the 3 tracked files deleted AGAIN (third time) despite the Exp 231 cwd fix; request for `--no-prompt` on all example scripts; full QA re-run from clean slate PASSED (chained deploys green after Exp 232); CI still failing on both jobs' unit test step; PR description update per the update-pr-description prompt.
- **The 3-file bug — actual root cause found at last.** `_git_tracked_files` returned `git ls-files` output as-is: REPO-RELATIVE Paths (`beat_saber_deluxe/custom_songs/fsb5_header_template.bin`). The wipe compared them against `dp.rglob("*")`'s ABSOLUTE paths with `item not in tracked` — `Path(relative) != Path(absolute)` for ANY file, so the preserve branch unlinked EVERYTHING (tracked included) while printing "preserved git-tracked: ..." (the log LIED). Exp 229 (cwd pin) and Exp 231 (cwd fix v2) both tested the HELPER in isolation — the helper worked; the COMPARISON never did. Lesson twice over: (a) test the CONSUMER of a helper, not the helper; (b) Path equality across normalization forms silently never matches — compare resolved-absolute strings. **Fix (both layers):** helper returns `(repo_root / line).resolve()`; wipe builds `tracked_abs = {str(tf.resolve())}` and compares `str(item.resolve()) not in tracked_abs`. **New E2E test** (`tests/test_clean_slate_wipe.py`) runs the REAL `clear_local_pipeline_state` against a scratch git repo — the first draft's fixture accidentally committed the artifacts, the test caught it (artifacts "preserved"), fixture fixed, E2E now proves: tracked survive / artifacts removed / dirs pruned. Also verified from 3 cwds against the real repo: comparison-match=True everywhere (was False — the bug).
- **--no-prompt:** all 34 example_*.sh scripts now accept `--no-prompt` — the pre-deploy `read` is skipped (3s announce), enabling unattended chained runs. Default unchanged. Decision block extracted and verified in isolation; syntax-checked all scripts.
- **CI failure (both jobs, same single test):** `TestPackModesRealArtifacts::test_real_entries_and_redirects` — `assert 0 == (0+1)`: the class asserts on the developer's real workspace state (redirects.json + locally-built bundles — gitignored, absent on CI runners) → `_resolve_active_packs` = [] → contradictory count assertions. This class was missed by the Exp 229 gates (different file, different skip pattern). All 4 tests in the class now carry the hardware-environment skip gate; verified: skips when pack_modes_bundles absent, passes in the real workspace.
- **QA status (user-reported):** full test procedure from clean slate PASSED — chained multi-pack deploy green (validating Exp 232's fix on hardware), everything working.
