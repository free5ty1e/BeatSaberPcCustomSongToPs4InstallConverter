#!/usr/bin/env python3
"""Combined approach: inject into uncompressed block + fix CRC with alignment padding."""
...
def main():
    print("Espresso Pack Bundle — Combined Approach")
    ...
    # Compute current CRC (after injection, before padding correction)
    ...
    if target_delta == 0:
        ...
        # Use GF(2) linear algebra on alignment padding bytes to fix CRC
        # The padding is at offset 263, with L = original_size - 263 - padding_size bytes after it
        ...
        # For each padding byte at offset (263 + i), bytes_after = original_size - (263 + i) - 1
        # Weight matrix W_i = M^(bytes_after) for byte i
        
        n = padding_size
        print(f"Computing weight matrices...")
        
        # Precompute M^k for k = 0 to original_size - 263 (too large, use binary representation)
        # Actually, we can compute on-the-fly since we process left to right
        
        target_gf2 = [(target_delta >> i) & 1 for i in range(32)]
        
        # Strategy: set all padding bytes to 0, then solve for corrections
        # For byte at position p with weight W_p: contribution = W_p * gf2(byte_val)
        # We need: sum(W_i * gf2(pad[i])) = target_gf2 (over GF(2))
        
        # Process from right to left (smallest weight first for stability)
        padding_values = [0] * n
        ...
        for i in range(n - 1, -1, -1):  # Right to left
            ...
            for d in range(256):
                ...
                # How much does this reduce remaining?
            ...
            print(f"  Pad byte {i}: value=0x{best_d:02x} (reduction={best_reduction})")
