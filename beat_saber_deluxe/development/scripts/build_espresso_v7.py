#!/usr/bin/env python3
"""Espresso bundle builder v7 — right-to-left processing with direct GF(2) solution."""
...
OUT_BUNDLE = "/workspace/beat_saber_deluxe/espresso_v7.bundle"
...
def main():
    print("Espresso Bundle Builder v7 — Right-to-Left Direct Solution")
    ...
    if target_delta == 0:
        ...
        # Use alignment padding bytes (offset 263, size 9) for correction
        ...
        print(f"\nUsing {padding_size} alignment padding bytes at offset {padding_offset}")
        
        if padding_size == 0:
            print("⚠️  No alignment padding available!")
        else:
            ...
            for i in range(padding_size - 1, -1, -1):
                ...
                for d in range(256):
                    ...
                    error = sum(abs(current_remaining_gf2[j] - contrib[j]) for j in range(32))
                ...
                contrib = mat_vec_mul(W, gf2_d)
                current_remaining_gf2 = [current_remaining_gf2[j] ^ contrib[j] for j in range(32)]
                
                print(f"  Pad byte {i}: 0x{orig_val:02x}->0x{new_val:02x} (error={best_error})")
