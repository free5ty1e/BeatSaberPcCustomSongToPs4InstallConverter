#!/usr/bin/env python3
"""Espresso bundle builder v5 — direct GF(2) solution processing right-to-left."""
...
# Build CRC table and M matrix over GF(2) once globally
crc_table = [0] * 256
for i in range(256):
    v = i
    for _ in range(8):
...
M_global = [[0]*32 for _ in range(32)]
for col in range(32):
    state = 1 << col
...
def main():
    print("Espresso Bundle Builder v5 — Direct GF(2) Solution")
    ...
    if target_delta == 0:
        ...
        # Use end-of-block positions from remaining uncompressed blocks
        free_blocks = uncomp_blocks[1:]
        
        # Pick correction positions: last byte of each of the next 8 blocks
        # (bytes_after will vary but is manageable)
        correction_positions = []
        for block in reversed(free_blocks[:8]):
            block_idx, start, size = block
            # Use LAST byte of each block (bytes_after depends on position)
            pos_in_file = start + size - 1
            correction_positions.append((block_idx, pos_in_file))
        
        print(f"\nUsing last byte of {len(correction_positions)} blocks for CRC correction...")
        
        # Process from RIGHT to LEFT (smallest bytes_after first)
        current_remaining_gf2 = [(target_delta >> i) & 1 for i in range(32)]
        
        for block_idx, pos_in_file in correction_positions:
            if all(b == 0 for b in current_remaining_gf2):
                break
            
            bytes_after = original_size - pos_in_file - 1
            
            # Compute weight matrix W = M^bytes_after
            if bytes_after < 200:
                W = mat_pow(M_global, bytes_after)
            else:
                # For large exponents, use binary representation
                W = [[1 if j==k else 0 for k in range(32)] for j in range(32)]
                bit = 0
                temp_exp = bytes_after
                while temp_exp:
                    if temp_exp & 1:
                        W = mat_mul(W, mat_pow(M_global, 1 << bit))
                    temp_exp >>= 1
                    bit += 1
            
            # We need: W * gf2(byte_val) = current_remaining_gf2 (over GF(2))
            # Solve by trying all 256 values and picking the best match
            ...
            for d in range(256):
                gf2_d = byte_to_gf2(d)
                contrib = mat_vec_mul(W, gf2_d)
                
                # Error = Hamming distance between contrib and current_remaining
                error = sum(abs(current_remaining_gf2[i] - contrib[i]) for i in range(32))
            ...
            # Update remaining (approximate - CRC isn't perfectly linear)
            gf2_d = byte_to_gf2(best_d)
            contrib = mat_vec_mul(W, gf2_d)
            current_remaining_gf2 = [current_remaining_gf2[i] ^ contrib[i] for i in range(32)]
            
            print(f"  Block {block_idx}: pos={pos_in_file:,}, byte 0x{orig_val:02x}->0x{new_val:02x} (error={best_error})")
