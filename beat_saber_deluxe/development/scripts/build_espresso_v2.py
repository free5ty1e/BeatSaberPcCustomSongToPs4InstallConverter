#!/usr/bin/env python3
"""Espresso bundle builder - inject into uncompressed block, fix CRC with end-of-file bytes."""
...
def main():
    print("Espresso Bundle Builder v2 — Uncompressed Injection + End-File CRC Fix")
    ...
    print(f"Injected {len(espresso_blob)}-byte blob into block {inj_block[0]} at offset {inj_block[1]:,}")
    ...
    if target_delta == 0:
        ...
        # Use bytes NEAR THE END of the file for CRC correction (smaller L = better)
        # Pick positions in the LAST uncompressed block (block 64 or similar)
        free_blocks = uncomp_blocks[1:]
        
        # Use last few blocks, near their end (bytes_after < 1000)
        correction_positions = []
        for block in reversed(free_blocks[:5]):  # Last 5 blocks
            block_idx, start, size = block
            # Use positions near the end of each block
            for offset_in_block in [size - 100, size - 200, size - 300]:
                if offset_in_block >= 0:
                    correction_positions.append((block_idx, start + offset_in_block))
        
        print(f"\nUsing {len(correction_positions)} correction positions near end of file...")
        
        # Greedy search with CRC verification at each step
        ...
        for block_idx, pos_in_file in correction_positions:
            ...
            print(f"  Pos {pos_in_file:,}: 0x{orig_val:02x}->0x{(orig_val+best_d)&0xFF:02x} (reduced {best_reduction} bits, remaining delta=0x{current_delta:08x})")
