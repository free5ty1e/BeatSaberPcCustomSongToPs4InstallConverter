...
**Plugin v0.8041 | Pipeline v0.5336** — Self-contained single-song `--deploy-full` with clean slate backup, feature flags gated, post-deploy validation, **surgical pack bundle patching for partial deploys**, **`--clear-target-song` for reverting slots**, **multi-song incremental deploy preserving all custom songs in a pack**.
